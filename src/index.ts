 ...
 *
 * Honest upstream: a nonexistent section 404s rather than returning an empty
 * page. The only wrinkle is the URL, which zero-pads the section to five
 * digits inside a title directory:
 *
 *   A.R.S. 13-1105  ->  /ars/13/01105.htm
 *
 * The padding is not something a caller would guess, which is the point of a
 * resolver.
 */
...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Arizona Revised Statutes');
}

const BASE = 'https://www.azleg.gov/ars';
const UA = 'pipeworx-mcp-arizona-code/1.0 (+https://pipeworx.io)';

const tools: McpToolExport['tools'] = [{
  name: 'az_revised_statute',
  description:
  ...
  inputSchema: {
    type: 'object',
    properties: {
      section: { type: 'string', description: 'Section as cited, e.g. "13-1105" or "28-1381". Title is the part before the dash.' },
    },
    required: ['section'],
  },
}];
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  if (name !== 'az_revised_statute') throw new Error(`Unknown tool: ${name}`);
  const raw = String(args.section ?? '').trim().replace(/\s+/g, '').replace(/^A\.?R\.?S\.?/i, '');
  const m = /^(\d+)-(\d+)([A-Za-z0-9.]*)$/.exec(raw);
  if (!m) {
    return { found: false, reason: 'unparsed_section',
      message: `"${args.section}" is not an Arizona citation.`,
      hint: 'Arizona sections are title-section, e.g. "13-1105" (murder 1) or "28-1381" (DUI).' };
  }
  const url = `${BASE}/${m[1]}/${m[2].padStart(5, '0')}${m[3] ?? ''}.htm`;
  const res = await pwFetch(url, { headers: { 'User-Agent': UA } });
  if (res.status === 404) {
    return { found: false, reason: 'section_not_found', section: raw, title: m[1],
      message: `No section ${raw} in the Arizona Revised Statutes (HTTP 404).`,
      hint: 'Section numbers are not contiguous — repealed sections are normal.' };
  }
  if (!res.ok) throw new Error(`Arizona Legislature returned HTTP ${res.status} for ${raw}`);
  const text = statuteText(await res.text());
  if (text.length < 40) {
    return { found: false, reason: 'empty_document', section: raw,
      message: `Section ${raw} resolved but returned no usable text — it may be repealed or reserved.` };
  }
  const heading = (text.split('\n')[0] ?? '').trim();
  return {
    citation: `A.R.S. § ${raw}`, section: raw, title: m[1],
    heading: heading || null, text, text_chars: text.length, url,
    source: 'Arizona State Legislature (azleg.gov), keyless',
    attribution: 'Arizona statutes are public record.',
  };
}
